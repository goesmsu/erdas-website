...
import sys, os, fnmatch, pprint, tarfile, shutil, stat

#Global constant basePath points to the data's base directory based on location of this python script
basePath = os.path.dirname(os.path.abspath(sys.argv[0]).replace("\\", "/"))
...
def FindModelerExe():
	...
	#for root, dirs, files in os.walk('C:\Program Files\Intergraph\\'):
	#	for dir in dirs:
	#		if dir == 'ERDAS IMAGINE 2013':
	...
	modeler = '"' + 'C:/Intergraph/ERDAS IMAGINE 2013/bin/Win32Release/modeler.exe' + '" @'
	imagecommand = '"' + 'C:/Intergraph/ERDAS IMAGINE 2013/bin/Win32Release/imagecommand.exe' + '" @'
	imgcopy = '"' + 'C:/Intergraph/ERDAS IMAGINE 2013/bin/Win32Release/imgcopy.exe' + '" @'
	return modeler, imagecommand, imgcopy

	
#extract compressed data if necessary
def ExtractData():
	targzList, tgzList, tarFileList, tarFolderList, untarFolderList = [], [], [], [], []
	...
	for root, dirs, files in os.walk(basePath):
		...
		for tgz in files:
			if fnmatch.fnmatch(tgz, '*.tgz'):
				tgzList.append(os.path.join(root, tgz).replace('\\', '/'))
		for tars in files:
	...
	combineLists = targzList + tgzList + tarFileList + tarFolderList + untarFolderList
	...
	for i in range(0, len(targzList)):		
		if not os.path.exists(basePath + '/Raw_data/' + str(targzList[i])[-28:-6]):
			os.makedirs(basePath + '/Raw_data/' + str(targzList[i])[-28:-6])
			tar = tarfile.open(basePath + '/Raw_data/' + os.path.basename(targzList[i]), 'r:gz')
			tar.extractall(basePath + '/Raw_data/' + str(targzList[i])[-28:-6])
			tar.close()
	for i in range(0, len(targzList)):		
		if not os.path.exists(basePath + '/Stacks/' + str(targzList[i])[-28:-6]):
			os.makedirs(basePath + '/Stacks/' + str(targzList[i])[-28:-6])
	for i in range(0, len(targzList)):
		if not os.path.exists(basePath + '/Toa_ref/' + str(targzList[i])[-28:-6]):
			os.makedirs(basePath + '/Toa_ref/' + str(targzList[i])[-28:-6])
	for i in range(0, len(targzList)):
		if not os.path.exists(basePath + '/NDVI/' + str(targzList[i])[-28:-6]):
			os.makedirs(basePath + '/NDVI/' + str(targzList[i])[-28:-6])
	for i in range(0, len(targzList)):
		if not os.path.exists(basePath + '/MSAVI/' + str(targzList[i])[-28:-6]):
			os.makedirs(basePath + '/MSAVI/' + str(targzList[i])[-28:-6])
	for i in range(0, len(targzList)):
		if not os.path.exists(basePath + '/static/' + str(targzList[i])[-28:-6]):
			os.makedirs(basePath + '/static/' + str(targzList[i])[-28:-6])
			
	for i in range(0, len(tgzList)):
		if not os.path.exists(basePath + '/Raw_data/' + str(tgzList[i])[-25:-3]):
			os.makedirs(basePath + '/Raw_data/' + str(tgzList[i])[-25:-3])
			tar = tarfile.open(basePath + '/Raw_data/' + os.path.basename(tgzList[i]), 'r')
			tar.extractall(basePath + '/Raw_data/' + str(tgzList[i])[-25:-3])
			tar.close()
	for i in range(0, len(tgzList)):
		if not os.path.exists(basePath + '/Stacks/' + str(tgzList[i])[-25:-3]):
			os.makedirs(basePath + '/Stacks/' + str(tgzList[i])[-25:-3])
	for i in range(0, len(tgzList)):
		if not os.path.exists(basePath + '/Toa_ref/' + str(tgzList[i])[-25:-3]):
			os.makedirs(basePath + '/Toa_ref/' + str(tgzList[i])[-25:-3])
	for i in range(0, len(tgzList)):
		if not os.path.exists(basePath + '/NDVI/' + str(tgzList[i])[-25:-3]):
			os.makedirs(basePath + '/NDVI/' + str(tgzList[i])[-25:-3])
	for i in range(0, len(tgzList)):
		if not os.path.exists(basePath + '/MSAVI/' + str(tgzList[i])[-25:-3]):
			os.makedirs(basePath + '/MSAVI/' + str(tgzList[i])[-25:-3])
	for i in range(0, len(tgzList)):
		if not os.path.exists(basePath + '/static/' + str(tgzList[i])[-25:-3]):
			os.makedirs(basePath + '/static/' + str(tgzList[i])[-25:-3])		
			
	for i in range(0, len(tarFileList)):
		if not os.path.exists(basePath + '/Raw_data/' + str(tarFileList[i])[-25:-3]):
			os.makedirs(basePath + '/Raw_data/' + str(tarFileList[i])[-25:-3])			
			tar = tarfile.open(basePath + '/Raw_data/' + os.path.basename(tarFileList[i]), 'r')
			tar.extractall(basePath + '/Raw_data/') #+ str(tarFileList[i])[-25:-3])
			tar.close()
	for i in range(0, len(tarFileList)):
		if not os.path.exists(basePath + '/Stacks/' + str(tarFileList[i])[-25:-3]):
			os.makedirs(basePath + '/Stacks/' + str(tarFileList[i])[-25:-3])
	for i in range(0, len(tarFileList)):
		if not os.path.exists(basePath + '/Toa_ref/' + str(tarFileList[i])[-25:-3]):
			os.makedirs(basePath + '/Toa_ref/' + str(tarFileList[i])[-25:-3])
	for i in range(0, len(tarFileList)):
		if not os.path.exists(basePath + '/NDVI/' + str(tarFileList[i])[-25:-3]):
			os.makedirs(basePath + '/NDVI/' + str(tarFileList[i])[-25:-3])
	for i in range(0, len(tarFileList)):
		if not os.path.exists(basePath + '/MSAVI/' + str(tarFileList[i])[-25:-3]):
			os.makedirs(basePath + '/MSAVI/' + str(tarFileList[i])[-25:-3])
	for i in range(0, len(tarFileList)):
		if not os.path.exists(basePath + '/static/' + str(tarFileList[i])[-25:-3]):
			os.makedirs(basePath + '/static/' + str(tarFileList[i])[-25:-3])
...
def GetParameterFilesStack(imagepath, imagename, modeler):
	...
	for i in range(0, numImages):
		if not os.path.exists(basePath + '/Stacks/' + imagename[i] + '_stack.img'):
			...
			if fnmatch.fnmatch(imagename[i], "LC8*"):
				...
				f.write('SET CELLSIZE MIN;\n'\
					...
					'INTEGER RASTER n8 FILE DELETE_IF_EXISTING PUBOUT USEALL ATHEMATIC 16 BIT UNSIGNED INTEGER"' + basePath + '/Stacks/' + imagename[i] + '/' + imagename[i] + '_stack.img";\n'
			...
			#create parameter file for landsat 7 bands
				...
				f.write('SET CELLSIZE MIN;\n'\
					...
					'INTEGER RASTER n5 FILE OLD PUBINPUT NEAREST NEIGHBOR AOI NONE EDGE FILL"'+ str(imagepath[i]) + str(imagename[i]) + '_b5.tif";\n'\
					'INTEGER RASTER n7 FILE OLD PUBINPUT NEAREST NEIGHBOR AOI NONE EDGE FILL"'+ str(imagepath[i]) + str(imagename[i]) + '_b7.tif";\n'\
					'INTEGER RASTER n8 FILE DELETE_IF_EXISTING PUBOUT USEALL ATHEMATIC 8 BIT UNSIGNED INTEGER"'+ basePath + '/Stacks/' + imagename[i] + '/' + imagename[i] + '_stack.img";\n'
	...
	for i in range (0, numImages):
		try:
			...
			for line in f:
				if "SUN_ELEVATION" in line: sunElev.append(str.lstrip(line[-12:-1]))
			f.close()
			strDOY = str(imagename[i])
			DOY.append(strDOY[13:16])
		except IOError:
			pass
	return DOY, sunElev
...
def getL4DNtoTOA(imagepath, imagename, grescale, brescale, sunelev, esdist, modeler):
	...
	batchlist = []
	for i in range(0, len(imagename)):
		if not os.path.exists(basePath + '/Toa_ref/' + imagename[i] + '/' + imagename[i] + '_toa.img'):
			try:
				Grescale = grescale[i]
				Brescale = brescale[i]
				...
				f.write('Integer RASTER n1 FILE OLD PUBINPUT "' + basePath + '/Stacks/' + imagename[i] + '/' + imagename[i] + '_stack.img";\n'\
				'Float RASTER n32 FILE DELETE_IF_EXISTING PUBOUT IGNORE 0 ATHEMATIC FLOAT SINGLE "' + basePath + '/Toa_ref/' + imagename[i] + '/' + imagename[i] + '_toa.img";\n'\
...
def MSAVI(imagepath, imagename, modeler):
	...
	batchlist = []
	for i in range (0, numImages):
		if not os.path.exists(basePath + '/MSAVI/' + imagename[i] + '/' + imagename[i] + '_msavi.img'):
			f = open(imagepath[i] + '/' + imagename[i] + '_msavi_paramA', 'w')
			f.write('Float RASTER n1 FILE OLD PUBINPUT "' + basePath + '/Toa_ref/' + imagename[i] + '/' + imagename[i] +'_toa.img";\n'\
			'Float RASTER n4_temp;\n'\
			'Float RASTER n5_temp;\n'\
			'Float RASTER n13 FILE DELETE_IF_EXISTING PUBOUT USEALL ATHEMATIC FLOAT SINGLE "' + basePath + '/MSAVI/' + imagename[i] + '/' + imagename[i] +'_msavi.img";\n'\
			'n5_temp = n1(4) * 1;\n'\
			'n4_temp = n1(3) * 1;\n'\
...
def NDVI(imagepath, imagename, modeler):
	...
	batchlist = []
	for i in range(0, numImages):
		if not os.path.exists(basePath + '/NDVI/' + imagename[i] + '/' + imagename[i] + '_ndvi.img'):
			f = open(imagepath[i] + imagename[i] + '_ndvi_paramA', 'w')
			f.write('Float RASTER n1 FILE OLD PUBINPUT NEAREST NEIGHBOR AOI NONE "' + basePath + '/Toa_ref/' + imagename[i] + '/' + imagename[i] + '_toa.img";\n'\
					'Float RASTER n5 FILE NEW PUBOUT USEALL ATHEMATIC FLOAT SINGLE "' + basePath + '/NDVI/' + imagename[i] + '/' + imagename[i] + '_ndvi.img";\n'\
...
def RunAtmCorrectBatch():
	try:
		...
		print "The batchfile wouldn't open"
		raise
		
def ImageConvertBatch(imagename):
	numImages = len(imagename)
	for i in range(0, numImages):
		f = open(basePath + '/convert_images.bat', 'w')
		f.write('cd C:/Users/bairdb/Desktop/website/mysite/MSAVI/' + str(imagename[i]) + '\n' + 'gdal_translate -of PNG -scale -ot byte ' + str(imagename[i]) + '_msavi' + '.img ' + str(imagename[i]) + '_msavi_thumb' + '.png' + '\n' + 'cd C:/Users/bairdb/Desktop/website/mysite/NDVI/' + str(imagename[i]) + '\n' + 'gdal_translate -of PNG -scale -ot byte ' + str(imagename[i]) + '_ndvi' + '.img ' + str(imagename[i]) + '_ndvi_thumb' + '.png')
	f.close()
def RunConvertBatch():
	try:
		from subprocess import Popen
		p = Popen("convert_images.bat", cwd=basePath)
		stdout, stderr = p.communicate()
	except WindowsError:
		print "The Image Conversion Batch File Wouldn't Open."
		raise
		
def calculateHistogramScript(imagename):
	numImages = len(imagename)
	for i in range(0, numImages):
		f = open('C:/Users/bairdb/Desktop/website/mysite/MSAVI/' + str(imagename[i]) + '/msavi_histogram.R', 'w')
		f.write(
		"library('raster')" '\n'
		"library('rgdal')" '\n'
		"library('ggplot2')" '\n'
		"landsat <- raster('" + str(imagename[i]) + "_msavi.img')" '\n'
		"samp <- sampleRandom(landsat, 100000, asRaster=FALSE)" '\n'
		"samp <- samp[samp != '0']" '\n'
		"samp <- data.frame(samp)" '\n'
		"names(samp)[1]<-paste('values')" '\n'
		"m <- ggplot(samp, aes(x=values))" '\n'
		"png(filename='C:/Users/bairdb/Desktop/website/mysite/static/" + str(imagename[i]) + "/msavi_histogram.png')" '\n'
		"m + geom_histogram(aes(fill = ..count..), binwidth=.001) + scale_fill_gradient('Count', low = 'green', high='red') + scale_x_continuous(limits=c(0, max(samp)), breaks=c(0.0,0.05,0.1,0.15,0.2,0.25,0.3,0.35,0.4,0.45,0.5,0.55,0.6,0.65,0.7,0.75,0.8,0.85,0.9,0.95,1.0))" '\n'
		"dev.off()"
		)
		shutil.copy('C:/Users/bairdb/Desktop/website/mysite/MSAVI/' + str(imagename[i]) + '/' + str(imagename[i]) + '_msavi_thumb.png', 'C:/Users/bairdb/Desktop/website/mysite/static/' + str(imagename[i]) + '/' + str(imagename[i]) + '_msavi_thumb.png')
		shutil.copy('C:/Users/bairdb/Desktop/website/mysite/NDVI/' + str(imagename[i]) + '/' + str(imagename[i]) + '_ndvi_thumb.png', 'C:/Users/bairdb/Desktop/website/mysite/static/' + str(imagename[i]) + '/' + str(imagename[i]) + '_ndvi_thumb.png')
	f.close()
	for i in range(0, numImages):
		g = open('C:/Users/bairdb/Desktop/website/mysite/NDVI/' + str(imagename[i]) + '/ndvi_histogram.R', 'w')
		g.write(
		"library('raster')" '\n'
		"library('rgdal')" '\n'
		"library('ggplot2')" '\n'
		"landsat <- raster('" + str(imagename[i]) + "_ndvi.img')" '\n'
		"samp <- sampleRandom(landsat, 100000, asRaster=FALSE)" '\n'
		"samp <- samp[samp != '0']" '\n'
		"samp <- data.frame(samp)" '\n'
		"names(samp)[1]<-paste('values')" '\n'
		"m <- ggplot(samp, aes(x=values))" '\n'
		"png(filename='C:/Users/bairdb/Desktop/website/mysite/static/" + str(imagename[i]) + "/ndvi_histogram.png')" '\n'
		"m + geom_histogram(aes(fill = ..count..), binwidth=.001) + scale_fill_gradient('Count', low = 'green', high='red') + scale_x_continuous(limits=c(0, max(samp)), breaks=c(0.0,0.05,0.1,0.15,0.2,0.25,0.3,0.35,0.4,0.45,0.5,0.55,0.6,0.65,0.7,0.75,0.8,0.85,0.9,0.95,1.0))" '\n'
		"dev.off()"
		)
	g.close()
	
def HistogramBatch(imagename):
	numImages = len(imagename)
	for i in range(0, numImages):
		f = open(basePath + '/genhist.bat', 'w')
		f.write('cd C:/Users/bairdb/Desktop/website/mysite/MSAVI/' + str(imagename[i]) + '\n'
				'Rscript msavi_histogram.R' '\n'
				'cd C:/Users/bairdb/Desktop/website/mysite/NDVI/' + str(imagename[i]) + '\n'
				'Rscript ndvi_histogram.R')
	f.close()

def RunHistogramBatch():
	try:
		from subprocess import Popen
		p = Popen("genhist.bat", cwd=basePath)
		stdout, stderr = p.communicate()
	except WindowsError:
		print "The Histogram Generation Batch File Wouldn't Open."
		raise
	
def main():
	
	print "Erdas Automation"
...
metadata fields and will not work with this script.\n'
	...
	batchHead1 = ['@echo off']
	batchHead2 = ['echo Content-Type: text/plain']
	batchHead3 = ['echo.']
	
	#compile all batch lists for atmospheric corrections and pass to the function that generates the batch file
	AtmCorrectBatchlist = batchHead1 + batchHead2 + batchHead3 + batchListStack + L8AtmCorBatchlist + L7AtmCorBatchlist + \
		L5AtmCorBatchlist + L4AtmCorBatchlist + batchListMSAVI + batchListNDVI
	...
	ImageConvertBatch(imagename)
	
	RunConvertBatch()
	
	calculateHistogramScript(imagename)
	
	HistogramBatch(imagename)
